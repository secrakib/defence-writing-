...
\begin{titlepage}
    ...
    \large
    \begin{tabular}{ccc}
        \textbf{Rakib Ullah} & \textbf{Ruhul Islam Rahul} & \textbf{Tanbir Ahmed} \\
    ...
    \vfill
    \includegraphics[width=0.22\textwidth]{graphics/sust-logo.png}
    \vfill
...
The foundation of our poly-dialectal NMT system is a meticulously curated multi-dialect parallel corpus. We integrate seven publicly available dialectal datasets, each contributing varying levels of coverage across different regional variants (see Table~\ref{tab:source_contributions}):
...
\addlinespace[2pt]
LoRA/DoRA Adapter & Rank ($r$) / Scaling ($\alpha$) & $r=64, \alpha=128$ & $r=64, \alpha=128$ & $r=64, \alpha=128$ \\
\addlinespace[2pt]
...
\addlinespace[2pt]
LoRA/DoRA Sweep & Ranks ($r$) / Scaling ($\alpha$) & \multicolumn{3}{l}{$r \in \{8, 64\}$ and $\alpha \in \{16, 128\}$} \\
\addlinespace[2pt]
...
All models are fine-tuned using \textbf{Low-Rank Adaptation (LoRA)} \cite{hu2021lora} and \textbf{Weight-Decomposed Low-Rank Adaptation (DoRA)} \cite{liu2024dora}, which are parameter-efficient fine-tuning (PEFT) methods. For a pre-trained weight matrix $W_0 \in \mathbb{R}^{d \times k}$, LoRA constrains the weight update $\Delta W$ to a low-rank decomposition:
\begin{equation}
h = W_0 x + \Delta W x = W_0 x + \frac{\alpha}{r} B A x
\end{equation}
where $A \in \mathbb{R}^{r \times k}$ and $B \in \mathbb{R}^{d \times r}$ are trainable low-rank matrices with rank $r \ll \min(d, k)$, and $\alpha$ is a scaling hyperparameter that controls the magnitude of the adaptation. This approach reduces the number of trainable parameters by over 99\% compared to full fine-tuning, enabling efficient adaptation on limited hardware.

To achieve learning capacity closer to full fine-tuning while maintaining parameter efficiency, we also utilize DoRA \cite{liu2024dora}. DoRA decomposes the pre-trained weight matrix $W_0$ into a magnitude vector $m \in \mathbb{R}^{1 \times k}$ and a directional matrix $V \in \mathbb{R}^{d \times k}$. The weight update is then formulated as:
\begin{equation}
W = m \frac{V + \Delta W}{\|V + \Delta W\|_c}
\end{equation}
where $\Delta W = \frac{\alpha}{r} B A$ represents the low-rank adaptation matrices, and $\|\cdot\|_c$ denotes the column-wise vector norm. By updating the magnitude and direction components independently, DoRA stabilizes optimization and outperforms standard LoRA in challenging low-resource translation tasks. With $r = 64$ and $\alpha = 128$ applied to all attention and feed-forward projections, the trainable parameter count is approximately 4.7M for BanglaT5 and 9.8M for NLLB-200 and mBART-50 (as shown in Figure~\ref{fig:lora_efficiency}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{graphics/lora_efficiency.pdf}
  \caption{Parameter efficiency of LoRA fine-tuning across the three evaluated model architectures, demonstrating over 98\% reduction in trainable parameters. Furthermore, we extend our methodology by introducing \textbf{Weight-Decomposed Low-Rank Adaptation (DoRA)} \cite{liu2024dora}, which decomposes the pre-trained weights into magnitude and directional components for fine-tuning. This approach maintains the parameter efficiency of LoRA while achieving learning capacity closer to full fine-tuning. A visual comparison distinguishing LoRA's structure from DoRA's decomposed optimization is provided in Figure~\ref{fig:lora_efficiency}.}
...
In the first phase, we evaluated three candidate NMT architectures---BanglaT5 \cite{bhattacharjee-etal-2023-banglanlg}, NLLB-200 \cite{nllb2022}, and mBART-50 \cite{tang2020multilingual}---to determine the optimal base model for poly-dialectal translation. All models were fine-tuned for \textbf{20 epochs} using identical LoRA \cite{hu2021lora} and DoRA \cite{liu2024dora} hyperparameters (rank $r=64$, $\alpha=128$) on the complete integrated dataset.
...
To quantify the relationship between training data volume and translation quality, we conducted a controlled scaling experiment using a fully parallel subset of four dialects (Sylheti, Chittagonian, Noakhali, Mymensingh) plus SCB, incrementally scaling from 500 to 4,499 sentence pairs under two LoRA \cite{hu2021lora}/DoRA \cite{liu2024dora} configurations ($r=8, \alpha=16$ and $r=64, \alpha=128$).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.90\textwidth]{graphics/dataset_scaling.pdf}
  \caption{BLEU \cite{papineni2002bleu} and chrF++ \cite{popovic2015chrf} as a function of training data volume for two LoRA \cite{hu2021lora}/DoRA \cite{liu2024dora} configurations. Consistent improvement is observed, with diminishing returns beyond approximately 3,000 samples.}
  \label{fig:scaling}
\end{figure}
...
\rowcolor{gray!15} \textbf{Proposed (Ours)} & \textbf{51,531 pairs} & \textbf{12 Dialects} & \textbf{BanglaT5 \cite{bhattacharjee-etal-2023-banglanlg} (100 ep)} & \textbf{29.26} & \textbf{57.26} \\
\bottomrule
\end{tabular}
}
\end{table}
