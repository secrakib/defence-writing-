...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive Evaluation of Multilingual NMT Architectures:} We systematically fine-tune and empirically evaluate three prominent Sequence-to-Sequence models---BanglaT5, NLLB-200, and mBART-50---under diverse, multi-directional translation settings using parameter-efficient Low-Rank Adaptation (LoRA).
...
\begin{enumerate}
    ...
    \item To systematically evaluate and compare the performance of three state-of-the-art multilingual NMT architectures---BanglaT5, NLLB-200, and mBART-50---under parameter-efficient LoRA fine-tuning on the constructed corpus.
...
We evaluate three state-of-the-art encoder-decoder Transformer architectures, chosen to represent a spectrum of design philosophies for multilingual and Bangla-specific NMT:
...
\textbf{BanglaT5} is a T5-based model pre-trained on approximately 27.5~GB of Bangla text, making it the only language-specific model in our comparison. \textbf{NLLB-200} (No Language Left Behind, distilled 600M variant) is Meta AI's massively multilingual model designed for 200 languages. \textbf{mBART-50} is Facebook AI's multilingual denoising auto-encoder covering 50 languages.

\subsection{Fine-Tuning Strategy}
All models are fine-tuned using \textbf{Low-Rank Adaptation (LoRA)}, a parameter-efficient method that injects trainable low-rank decomposition matrices into each attention layer while freezing the pre-trained weights. We investigate two LoRA configurations:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{BLEU} (Bilingual Evaluation Understudy): Measures $n$-gram precision between generated and reference translations.
    \item \textbf{chrF++}: Character-level $F$-score incorporating word-level bigrams, robust to morphological variations.
    \item \textbf{METEOR}: Harmonic mean of precision and recall with synonym matching and stemming.
    \item \textbf{TER} (Translation Edit Rate): Minimum number of edits required to transform the output into the reference (lower is better).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Absence of Human Evaluation:} While automatic metrics (BLEU, chrF++, METEOR, TER) provide objective and reproducible quality assessments, they cannot fully capture translation fluency, dialectal authenticity, and semantic adequacy as perceived by native speakers.
\end{enumerate}
